...
const G3_BATCH = [
  {
    gameId: '0-story-playstation-2',
    title: '0 Story',
    devTeam: [
      { role: 'Developer', name: 'General Entertainment' },
    ],
  },
  {
  ...
  },
  {
    ...
    devTeam: [
      { role: 'Developer', name: 'Bit Managers' },
    ],
  },
  {
    ...
    title: 'Body Harvest',
    devTeam: [
      { role: 'Developer', name: 'DMA Design' },
    ],
  },
  {
  ...
  },
  {
    ...
    title: "'98 Koshien",
    devTeam: [
      { role: 'Developer', name: 'Magical Company' },
    ],
  },
  {
...
function createRun(db, runKey, timestamp, dryRun) {
  const result = db.prepare(`
    INSERT INTO enrichment_runs (
      ...
      error_count,
      notes
    ) VALUES (?, 'g3_dev_team_batch_1', 'apply', 'internal_curated', 'running', ?, ?, 0, 0, 0, 0, 0, 0, ?)
  `).run(runKey, dryRun ? 1 : 0, timestamp, 'G3 batch 1 inferred developer team seed')
...
function applyBatch(db) {
  ...
  const runKey = `g3-dev-team-batch-1-${timestamp}`
  ...
  const metrics = {
    ...
    itemsSkipped: 0,
    itemsFlagged: 0,
    notes: 'G3 dev team batch 1 applied locally on staging sqlite',
