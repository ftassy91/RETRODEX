...
const G3_BATCH = [
  ...
  },
  {
    ...
    title: 'Ah Nanjarin',
    devTeam: [
      { role: 'Developer', name: 'Whiteboard' },
    ],
  },
  {
    ...
    devTeam: [
      { role: 'Developer', name: 'Video System' },
    ],
  },
  {
  ...
  },
  {
    ...
    devTeam: [
      { role: 'Developer', name: 'Game Studio' },
    ],
  },
  {
    ...
    devTeam: [
      { role: 'Developer', name: 'Crawfish Interactive' },
    ],
  },
  {
    gameId: 'all-star-racing-playstation',
    title: 'All Star Racing',
    devTeam: [
      { role: 'Developer', name: 'Mud Duck Productions' },
    ],
  },
  {
...
function createRun(db, runKey, timestamp, dryRun) {
  const result = db.prepare(`
    INSERT INTO enrichment_runs (
      ...
      error_count,
      notes
    ) VALUES (?, 'g3_dev_team_batch_3', 'apply', 'internal_curated', 'running', ?, ?, 0, 0, 0, 0, 0, 0, ?)
  `).run(runKey, dryRun ? 1 : 0, timestamp, 'G3 batch 3 inferred developer team seed')
...
function applyBatch(db) {
  ...
  const runKey = `g3-dev-team-batch-3-${timestamp}`
  ...
  const metrics = {
    ...
    itemsSkipped: 0,
    itemsFlagged: 0,
    notes: 'G3 dev team batch 3 applied locally on staging sqlite',
