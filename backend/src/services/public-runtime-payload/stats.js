...
const franchiseCountCache = new LRUCache(1, 5 * 60 * 1000)
let statsPromise = null
let franchiseCountPromise = null

async function fetchFranchiseCount() {
  const cached = franchiseCountCache.get('franchise-count')
  if (cached != null) {
    return cached
  }

  if (franchiseCountPromise) {
    return franchiseCountPromise
  }

  franchiseCountPromise = (async () => {
    const { count: rawFranchiseCount, error: franchiseError } = await db
      .from('franchise_entries')
      .select('*', { count: 'exact', head: true })

    if (franchiseError) {
      if (isMissingSupabaseRelationError(franchiseError)) {
        franchiseCountCache.set('franchise-count', 0)
        return 0
      }
      throw new Error(franchiseError.message)
    }

    const count = Number(rawFranchiseCount || 0)
    franchiseCountCache.set('franchise-count', count)
    return count
  })()

  try {
    return await franchiseCountPromise
  } finally {
    franchiseCountPromise = null
  }
}
...
async function fetchStatsPayload() {
  ...
  statsPromise = (async () => {
    const [games, franchiseCount] = await Promise.all([
