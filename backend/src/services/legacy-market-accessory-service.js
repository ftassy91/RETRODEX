...
function toLegacyAccessoryPayload(item, consoleTitleMap) {
  return {
    id: item.id,
    name: item.name,
...
async function listLegacyConsoleAccessories(consoleId, { limit = 10 } = {}) {
...
async function listLegacyAccessoryTypes() {
  const accessories = await Accessory.findAll({
...
async function listLegacyAccessories() {
  const accessories = await Accessory.findAll({
    order: [['name', 'ASC']],
  })
  ...
  return {
    accessories: accessories.map((item) => toLegacyAccessoryPayload(item, consoleTitles)),
