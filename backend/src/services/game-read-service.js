...
const gameCache = new LRUCache(200, 300000) // 200 entries, 5 min TTL
...
async function queryIfTableExists(tableName, queryFn) {
  const exists = await tableExists(tableName)
  return exists ? queryFn() : []
}

async function queryIfAllTablesExist(tableNames, queryFn) {
  const results = await Promise.all(tableNames.map((name) => tableExists(name)))
  return results.every(Boolean) ? queryFn() : []
}
...
async function loadCanonicalSupplements(gameId) {
  const [editorialRows, peopleRows, mediaRows, snapshotRows, qualityRows, releaseRows] = await Promise.all([
    queryIfTableExists('game_editorial', () => sequelize.query(
      `SELECT summary,
              synopsis,
              lore,
    ...
    queryIfAllTablesExist(['game_people', 'people'], () => sequelize.query(
    ...
    queryIfTableExists('media_references', () => sequelize.query(
    ...
    queryIfTableExists('market_snapshots', () => sequelize.query(
    ...
    queryIfTableExists('quality_records', () => sequelize.query(
    ...
    queryIfTableExists('releases', () => sequelize.query(
...
async function getHydratedGameByLookup(lookup) {
  ...
  const cached = gameCache.get(needle)
  if (cached !== undefined) {
    return cached
  }
