...
function validateReportPayload(body = {}) {
  ...
  if (!Number.isFinite(normalizedPrice) || normalizedPrice <= 0) {
    throw createValidationError('reported_price doit être supérieur à 0')
  }

  if (normalizedDate && !/^\d{4}-\d{2}-\d{2}$/.test(normalizedDate)) {
    throw createValidationError('date_estimated doit être au format YYYY-MM-DD ou null')
  }
...
async function createLegacyMarketReport(body = {}) {
  const payload = validateReportPayload(body)
  const newReport = await CommunityReport.create(payload)
