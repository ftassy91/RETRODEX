const heroEl = document.getElementById("hero");
      ...
      function generateCoverPlaceholder(title, rarity, consoleName) {
        ...
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
          ...
          <text x="80" y="78" text-anchor="middle" font-family="monospace" font-size="42" font-weight="bold" fill="#00ff66" opacity="0.8">${initials || "?"}</text>
          <text x="80" y="100" text-anchor="middle" font-family="monospace" font-size="9" fill="#2e4e2e">${platformLabel}</text>
        </svg>`;
              ...
              <div class="game-header-main">
                <div class="game-cover-container">
                  <img
                    ...
                    width="160"
                    height="160"
                    onerror="this.src=generateCoverPlaceholder(currentGame.title, currentGame.rarity, currentGame.console)"
                  />
                </div>
                <div class="game-header-copy">
                  ...
                  <div class="game-console-link">
                    Console :
                    <a class="console-link" href="/consoles.html?platform=${encodeURIComponent(game.console || "")}">${escapeHtml(game.console || "Console inconnue")} &rarr;</a>
                  </div>
                  <div class="developer-line">Développé par ${escapeHtml(game.developer || "studio inconnu")}</div>
                </div>
              </div>
            </div>
            <aside class="price-panel">
              <div id="retrodex-index" class="index-insufficient">Chargement de l'indice RetroDex...</div>
              <div class="encyclo-section" id="encyclo-section" style="display:none">
                <div class="encyclo-header">
                  <span class="encyclo-label">ENCYCLOPÉDIE</span>
                </div>
                <div class="encyclo-block" id="encyclo-synopsis-block">
                  <div class="section-label">SYNOPSIS</div>
                  <div class="encyclo-text" id="encyclo-synopsis"></div>
                </div>
                <div class="encyclo-block" id="encyclo-team-block">
                  <div class="section-label">ÉQUIPE DE DÉVELOPPEMENT</div>
                  <div id="encyclo-team"></div>
                </div>
                <div class="encyclo-block" id="encyclo-anecdotes-block">
                  <div class="section-label">ANECDOTES</div>
                  <div id="encyclo-anecdotes"></div>
                </div>
                <div class="encyclo-block" id="encyclo-cheats-block">
                  <div class="section-label">CODES & ASTUCES</div>
                  <div id="encyclo-cheats"></div>
                </div>
                <div class="similar-section" id="similar-section" style="display:none">
                  <div class="section-label">JEUX SIMILAIRES</div>
      ...
      function buildTrustSource(entries, confidence) {
        ...
        if (sourcesEditorial > 0) {
          ...
          return formattedDate
            ? `${sourcesEditorial} ventes réelles · dernière : ${formattedDate}`
            : `${sourcesEditorial} ventes réelles`;
        }

        if ((Number(confidence) || 0) === 15) {
          return "estimation statistique — aucune vente vérifiée";
        }
      ...
      async function loadRetrodexIndex(gameId) {
        ...
        try {
          ...
          const entries = Array.isArray(payload.index) ? payload.index : [];
          ...
          if (!entries.length || !hasUsableIndex) {
            ...
            indexEl.textContent = "Données insuffisantes — contribuez un prix";
            return;
          }
          ...
          indexEl.innerHTML = `
            <div class="index-header">
              <span>INDICE RETRODEX</span>
              <span class="confidence ${confidenceClass(confidence)}">${confidence}% confiance</span>
            </div>
            <div class="trust-header">
              <span class="trust-badge trust-${trustMeta.tier}">${trustMeta.label}</span>
            ...
            <div class="index-prices">
              ${orderedEntries.map((entry) => `
                <div class="index-condition">
                  ...
                  <span class="value">${escapeHtml(formatIndexValue(entry.index_value))}</span>
                  <span class="range">${escapeHtml(formatIndexRange(entry.range_low, entry.range_high))}</span>
                </div>
              `).join("")}
            </div>
            <div class="index-sources">
              ${sourcesEditorial} ventes · 0 listings · ${sourcesCommunity} contributions
            </div>
          `;
        } catch (_error) {
          indexEl.className = "index-insufficient";
      ...
      async function loadEncyclopedia(gameId) {
        try {
          const res = await fetch(`/api/games/${gameId}/encyclopedia`);
          const data = await res.json();
          if (!data.ok) return;

          const hasContent = data.synopsis || data.dev_team?.length ||
            data.dev_anecdotes?.length || data.cheat_codes?.length;
          if (!hasContent) return;

          document.getElementById("encyclo-section").style.display = "block";

          if (data.synopsis) {
            document.getElementById("encyclo-synopsis").textContent = data.synopsis;
          } else {
            document.getElementById("encyclo-synopsis-block").style.display = "none";
          }

          const teamEl = document.getElementById("encyclo-team");
          if (data.dev_team?.length) {
            data.dev_team.forEach((m) => {
              const row = document.createElement("div");
              row.className = "encyclo-team-row";
              row.innerHTML = `<span class="team-role">${m.role}</span><span class="team-name">${m.name}</span>`;
              teamEl.appendChild(row);
            });
          } else {
            document.getElementById("encyclo-team-block").style.display = "none";
          }

          const anecEl = document.getElementById("encyclo-anecdotes");
          if (data.dev_anecdotes?.length) {
            data.dev_anecdotes.forEach((a) => {
              const block = document.createElement("div");
              block.className = "encyclo-anecdote";
              block.innerHTML = `<div class="anecdote-title">${a.title}</div><div class="anecdote-text">${a.text}</div>`;
              anecEl.appendChild(block);
            });
          } else {
            document.getElementById("encyclo-anecdotes-block").style.display = "none";
          }

          const cheatEl = document.getElementById("encyclo-cheats");
          if (data.cheat_codes?.length) {
            data.cheat_codes.forEach((c) => {
              const row = document.createElement("div");
              row.className = "encyclo-cheat-row";
              row.innerHTML = `<span class="cheat-name">${c.name}</span><span class="cheat-code">${c.code}</span><span class="cheat-effect">${c.effect}</span>`;
              cheatEl.appendChild(row);
            });
          } else {
            document.getElementById("encyclo-cheats-block").style.display = "none";
          }
        } catch (e) {
          console.warn("Encyclopedia load failed:", e.message);
        }
      }

      async function loadFranchise(gameId) {
        try {
          const res = await fetch(`/api/games/${gameId}/franchise`);
          const data = await res.json();
          if (!data.ok || !data.franchise) return;

          const f = data.franchise;
          const existing = document.querySelector(".game-franchise-link");
          if (existing) {
            existing.remove();
          }

          const el = document.createElement("div");
          el.className = "game-franchise-link";
          el.innerHTML = `Franchise : <a class="franchise-link" href="/franchises.html?slug=${encodeURIComponent(f.slug)}">${escapeHtml(f.name)} (${escapeHtml(f.first_game || "—")}→${escapeHtml(f.last_game || "—")}) →</a>`;

          const badges = document.querySelector(".game-badges") || document.querySelector(".game-meta");
          if (badges) {
            badges.after(el);
          }
        } catch (_error) {}
      ...
      function renderSummary(game) {
        summaryEl.textContent = game.summary || "Aucun résumé éditorial disponible.";
      }

      function renderStats(game) {
        const stats = [
          ["Metascore", game.metascore || "—"],
          ["Rareté", game.rarity || "COMMON"],
          ["Développeur", game.developer || "n/a"],
          ["Année", game.year || "n/a"],
        ];
      ...
      async function refreshCollectionStatus() {
        ...
        try {
          ...
          currentCollectionItem = (payload.items || []).find((item) => item.gameId === currentGame.id) || null;
          if (currentCollectionItem) {
            collectionStateEl.textContent = "Dans votre collection ✓";
            collectionCurrentMetaEl.innerHTML = `
              <span class="condition-badge condition-${escapeHtml(conditionClass(currentCollectionItem.condition))}">${escapeHtml(currentCollectionItem.condition || "Loose")}</span>
            ...
            collectionButtonEl.textContent = "Ajouter à ma collection";
            collectionButtonEl.disabled = false;
          }
        } catch (error) {
      ...
      async function handleCollectionAction() {
        ...
        try {
          if (currentCollectionItem) {
            ...
            collectionStatusEl.textContent = "Jeu retiré de votre collection.";
          } else {
            ...
            collectionStatusEl.textContent = "Jeu ajouté à votre collection.";
          }
      ...
      async function handleContributionSubmit() {
        if (!currentGame) {
          setContributionFeedback("Impossible de soumettre sans fiche chargée.", true);
          return;
        }
      ...
      function removeRelatedPricesSection() {
        document.querySelector(".price-compare")?.remove();
      }
      ...
      function renderRelatedPrices(current, related) {
        ...
        section.innerHTML = `
          <h3>MEME FRANCHISE - AUTRES VERSIONS</h3>
          <div class="compare-table">
            ...
            ${related.map((game) => `
              <div class="compare-row clickable" onclick="window.location='/game-detail.html?id=${encodeURIComponent(game.id)}'" style="cursor:pointer">
        ...
        detailShellEl.appendChild(section);
      }

      async function loadRelatedGames(currentGame) {
        removeRelatedPricesSection();
        const series = extractSeries(currentGame?.title);
        ...
        try {
          ...
          const currentTitle = String(currentGame.title || "").toLowerCase();
          ...
          const related = (data.items || [])
            ...
            .filter((game) => String(game.title || "").toLowerCase().includes(normalizedSeries))
            .sort((left, right) => {
              const score = (game) => {
                ...
                if (String(game.console || "").toLowerCase() === String(currentGame.console || "").toLowerCase()) total += 15;
          ...
          renderRelatedPrices(currentGame, related);
        } catch (error) {
      ...
      async function loadSimilar(gameId) {
        try {
          const res = await fetch(`/api/games/${gameId}/similar`);
          const data = await res.json();
          ...
          data.games.forEach((g) => {
            ...
            item.onclick = () => { location.href = `/game-detail.html?id=${g.id}`; };
            item.innerHTML = `
              <div class="similar-title">${escapeHtml(g.title)}</div>
              <div class="similar-meta">${escapeHtml(g.console || "—")} · ${escapeHtml(g.year || "—")}</div>
              <div class="similar-price">${Number(g.loosePrice) ? `$${Math.round(Number(g.loosePrice))}` : "—"}</div>
            `;
        ...
        function drawChart(type) {
          ...
          const points = prices.map((price, index) => {
            const x = prices.length === 1
              ? width / 2
              : pad + (index / denominator) * (width - pad * 2);
